...
Generated on: 7/13/2026, 4:24:09 AM
...
app.get('/api/photos', async (req, res) => {
  ...
  if (unsplashKey && unsplashKey !== 'your_unsplash_access_key_here') {
    try {
      ...
      console.error('[UNSPLASH ERROR] Failed to fetch photo from Unsplash:', error.message);
    }
  }

  // Fallback to LoremFlickr for beautiful, keyless query-based images
  const keywords = query.trim().split(/\s+/).join(',');
  const fallbackUrl = `https://loremflickr.com/800/600/${keywords}`;
...
class AppConfig {
  ...
  static String get proxyBaseUrl {
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      // ملاحظة: 10.0.2.2 يعمل للمحاكي فقط. للهاتف الحقيقي استخدم IP شبكتك (مثال 192.168.100.7)
      // الأفضل استخدام: flutter run --dart-define=PROXY_BASE_URL=http://192.168.100.7:3000
            ...
            child: GestureDetector(
              ...
              child: GlassCard(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    if (stop.imageUrl != null && stop.imageUrl!.isNotEmpty)
                      ClipRRect(
                        borderRadius: const BorderRadius.only(
...
## File: `lib/features/saved_trips/presentation/cubit/saved_trips_cubit.dart`
```dart
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:equatable/equatable.dart';
import '../../../trip_planner/domain/repositories/trip_repository.dart';
...
## File: `lib/features/saved_trips/presentation/cubit/saved_trips_state.dart`
```dart
...
## File: `lib/features/saved_trips/presentation/screens/saved_trips_screen.dart`
```dart
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:flutter_animate/flutter_animate.dart';
  ...
  void _confirmDelete(BuildContext context) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          AppStrings.of(context).deleteTripConfirm(trip.destination),
          style: AppTextStyles.bodyMedium,
        ),
        actions: [
          TextButton(
  ...
  Future<Either<Failure, void>> saveTrip({
    ...
    try {
      await _dbHelper.executeInTransaction((txn) async {
        ...
        // 2. Insert days, stops, and collect recommended restaurants
        final Map<int, String> dayIdMap = {};
        final Map<String, Map<String, dynamic>> restaurantsToInsert = {};

        for (final dayPlan in planResponse.days) {
          final dayId = _uuid.v4();
          dayIdMap[dayPlan.dayNumber] = dayId;
          final dayDate =
          ...
          for (final stop in dayPlan.stops) {
            final hasValidCoords =
                stop.latitude.abs() > 0.001 && stop.longitude.abs() > 0.001;
            if (!hasValidCoords) {
              debugPrint(
                  '⚠️ TripRepository: Stop "${stop.name}" has invalid coordinates (${stop.latitude}, ${stop.longitude})');
            }

            // Fetch image for this stop
            String? stopImageUrl;
            try {
              ...
              stopImageUrl = await sl<ImageSearchService>().searchPhoto(query);
            } catch (e) {
              debugPrint('⚠️ TripRepository: Failed to fetch image for stop "${stop.name}": $e');
            }
          ...
          if (rec != null) {
            ...
            // Fetch image for recommended restaurant
            String? recImageUrl;
            try {
              ...
              recImageUrl = await sl<ImageSearchService>().searchPhoto(query);
            } catch (e) {
              debugPrint('⚠️ TripRepository: Failed to fetch image for restaurant "${rec.name}": $e');
            }
        ...
        // 3. Add general restaurants, skip if already added as recommended
        for (final r in planResponse.allRestaurants) {
          final key = r.name.trim().toLowerCase();
          if (!restaurantsToInsert.containsKey(key)) {
            // Fetch image for general restaurant
            String? rImageUrl;
            try {
              final query = (r.imageSearchQuery != null && r.imageSearchQuery!.isNotEmpty)
                  ? r.imageSearchQuery!
                  : '${r.name} restaurant ${trip.destination}';
              rImageUrl = await sl<ImageSearchService>().searchPhoto(query);
            } catch (e) {
              debugPrint('⚠️ TripRepository: Failed to fetch image for restaurant "${r.name}": $e');
            }
            restaurantsToInsert[key] = {
              'id': _uuid.v4(),
              'trip_id': trip.id,
        ...
        // 4. Insert all deduplicated restaurants
        for (final row in restaurantsToInsert.values) {
          await txn.insert('restaurants', row,
              conflictAlgorithm: ConflictAlgorithm.replace);
        }
  ...
  Future<void> _shareTrip() async {
    ...
    try {
      ...
      await Share.shareXFiles(
        [XFile(file.path)],
        text: '${strings.appName} — ${trip.destination}\n${strings.appTagline}',
...
class CachedHeroImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return CachedNetworkImage(
      ...
      width: double.infinity,
      fit: fit,
      placeholder: (context, url) => placeholder?.call() ?? 
        ShimmerBox(height: height ?? 160),
      errorWidget: (context, url, error) => placeholder?.call() ?? 
...
class ShimmerBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Shimmer.fromColors(
      baseColor: AppColors.adaptiveBgCard(context),
      highlightColor: AppColors.adaptiveBgCard(context).withValues(alpha: 0.5),
      child: Container(
